...
public class PlacementBar : MonoBehaviour
{
    ...
    public void InsertToSlug(GameObject obj) {
        ...
        int count = mj.type == Combination.Single ? 1 : 2;
        ...
        if(removeType == Match.Chow || removeType == Match.Pung) {
            placeCount -= 3;
        } else if(removeType == Match.Kong) {
            placeCount -= 4;
        }
        if(removeType != Match.None) {
    ...
    private void RemoveMahjongs(GameObject left, GameObject right) {
        ...
        GameManager.Instance.RemoveAndRematch(left, right);
    }
